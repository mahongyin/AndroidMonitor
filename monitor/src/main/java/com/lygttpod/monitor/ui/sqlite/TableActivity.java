...
import com.lygttpod.monitor.data.Cell;
...
public class TableActivity extends Activity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        String path = getIntent().getStringExtra(EXTRA_DATABASE_PATH);
    ...
    private String[][] buildArray() {
        ...
        for (int i = 0; i < cursor.getColumnCount(); i++) {
            ...
            for (int j = 0; j < cursor.getCount(); j++) {
                ...
                if (c.getString(index).equals("BLOB")) {
                    data[i][j] = "[BLOB]";
                } else {
                    int columnIndex = cursor.getColumnIndex(mColumnNames[i]);
                    data[i][j] = cursor.getString(columnIndex);
                }
            }
        }
    ...
    private String buildSQL(int col, int row, String name, String newValue) {
        ...
        String sql = cursor.getString(index);
        cursor.close();
        ...
        int columnIndex = c.getColumnIndex(primaryKey);
        String key = c.getString(columnIndex);
        int columnIndex1 = c.getColumnIndex(name);
        String value = c.getString(columnIndex1);
        c.close();
    ...
    private class OnItemClickListener implements TableData.OnItemClickListener {
        ...
        public void onClick(Column column, String value, Object o, int col, int row) {
            ...
            int columnIndex1 = cursor.getColumnIndex("type");
            String type = cursor.getString(columnIndex1);
            int columnIndex2 = cursor.getColumnIndex("notnull");
            boolean notNull = cursor.getInt(columnIndex2) == 1;
            cursor.close();
